...
function openCreate() { editing.value = null; form.value = { name: '', plaintext: '', priority: 100, weight: 100, remark: '' }; editorOpen.value = true }
function openEdit(r: ProviderCredentialSummary) { editing.value = r; form.value = { name: r.name, plaintext: '', priority: r.priority, weight: r.weight, remark: r.remark || '' }; editorOpen.value = true }
  ...
  <div class="credential-panel">
    ...
    <n-modal v-model:show="editorOpen" preset="card" :title="editing ? '编辑凭证' : '新增凭证'" style="max-width:480px">
      <n-form label-placement="top">
        ...
        <n-form-item v-if="!editing" label="上游凭证" required><n-input v-model:value="form.plaintext" type="password" show-password-on="click" placeholder="输入上游访问凭证" /><template #feedback>创建后不可查看明文，仅保留脱敏标识。</template></n-form-item>
