...
const form = reactive({
  ...
  providerBaseUrlId: null as number | null,
  name: '', priority: 100, weight: 100,
  connectTimeoutMs: 5000, readTimeoutMs: 60000, remark: '',
})
...
async function loadList() {
  ...
  try {
    const r = await listProviderChannels({
      ...
      tenantId: auth.isPlatformAdmin ? (tenantFilter.value ?? undefined) : undefined,
      page: page.value, size,
    })
    rows.value = r.items
    total.value = r.total
  } catch (e: any) { message.error(e.userMessage || '加载通道失败，请稍后重试') }
  finally { loading.value = false }
}

async function loadStats() {
  statsLoading.value = true
  try { stats.value = await getProviderChannelStats(auth.isPlatformAdmin ? (tenantFilter.value ?? undefined) : undefined) }
  catch { /* 静默 */ }
...
async function openEdit(row: ProviderChannelSummary) {
  editing.value = row
  Object.assign(form, {
    tenantId: row.tenantId, providerId: row.providerId, providerBaseUrlId: row.providerBaseUrlId,
    name: row.name, priority: row.priority, weight: row.weight,
    connectTimeoutMs: row.connectTimeoutMs, readTimeoutMs: row.readTimeoutMs, remark: row.remark || '',
  })
  if (row.providerId) { try { formBaseUrls.value = await listProviderBaseUrls(row.providerId) } catch { formBaseUrls.value = [] } }
  editorOpen.value = true
}
...
async function save() {
  ...
  try {
    const payload = {
      tenantId: auth.isPlatformAdmin ? form.tenantId : null,
      providerBaseUrlId: form.providerBaseUrlId, name: form.name.trim(),
      priority: form.priority, weight: form.weight,
      connectTimeoutMs: form.connectTimeoutMs, readTimeoutMs: form.readTimeoutMs,
      remark: form.remark.trim() || undefined,
    }
    if (editing.value) await updateProviderChannel(editing.value.id, payload)
    else await createProviderChannel(payload)
    message.success(editing.value ? '通道已更新' : '通道已创建')
...
async function toggle(row: ProviderChannelSummary) {
  const enable = row.status !== 'ENABLED'
  try { await setProviderChannelEnabled(row.id, enable); message.success(enable ? '已启用' : '已停用'); await refreshAfterWrite() }
  catch (e: any) { message.error(e.userMessage || '状态更新失败，请稍后重试') }
}

function confirmDelete(row: ProviderChannelSummary) {
  dialog.warning({
    title: '删除通道', content: `确定删除「${row.name}」吗？仍被凭证引用时无法删除。`,
    positiveText: '删除', negativeText: '取消',
    onPositiveClick: async () => {
      try { await deleteProviderChannel(row.id); message.success('通道已删除'); await refreshAfterWrite() }
      catch (e: any) { message.error(e.userMessage || '删除失败，请稍后重试') }
    },
  })
}

async function openDetail(row: ProviderChannelSummary) {
  try { detailChannel.value = await getProviderChannel(row.id); detailOpen.value = true }
  catch (e: any) { message.error(e.userMessage || '加载通道详情失败，请稍后重试') }
}
...
const columns = computed(() => [
  { title: '通道', key: 'name', minWidth: 150, render: (r: ProviderChannelSummary) => h('button', { class: 'link-btn', onClick: () => openDetail(r) }, r.name) },
  ...(auth.isPlatformAdmin ? [{ title: '所属租户', key: 'tenantName', width: 130, render: (r: ProviderChannelSummary) => r.tenantName || '—' }] : []),
  { title: '上游厂商', key: 'providerName', width: 130 },
  { title: '协议', key: 'protocol', width: 90 },
  { title: '接入地址', key: 'normalizedBaseUrl', minWidth: 220, ellipsis: { tooltip: true } },
  ...
  { title: '操作', key: 'actions', fixed: 'right' as const, width: 200, render: (r: ProviderChannelSummary) => h('div', { class: 'row-actions' }, [
    h('button', { class: 'link-btn', disabled: !auth.canUpdateUpstream, onClick: () => openEdit(r) }, '编辑'),
    h('button', { class: 'link-btn', disabled: !auth.canEnableUpstream, onClick: () => toggle(r) }, r.status === 'ENABLED' ? '停用' : '启用'),
    h('button', { class: 'link-btn danger', disabled: !auth.canDeleteUpstream, onClick: () => confirmDelete(r) }, '删除'),
  ]) },
])
  ...
  <section class="channel-page">
    ...
    <div class="toolbar">
      <label class="search">
        ...
        <input v-model="keyword" class="search-input" type="text" placeholder="搜索通道名称" aria-label="搜索通道" @keyup.enter="page = 1; loadList()" />
    ...
    <div class="table-region">
      <n-data-table v-if="rows.length || loading" :columns="columns" :data="rows" :loading="loading" :pagination="false" :bordered="false" :single-line="false" :scroll-x="1100" flex-height size="medium" />
      <div v-else class="empty">
        <n-empty :description="hasFilter ? '没有符合筛选条件的通道' : '还没有上游通道'">
          <template #extra><n-button v-if="!hasFilter && auth.canCreateUpstream" type="primary" @click="openCreate">新增第一个通道</n-button></template>
        </n-empty>
      </div>
    </div>

    <footer class="page-foot">
      <span>共 <strong>{{ total }}</strong> 个通道</span>
    ...
    <n-modal v-model:show="editorOpen" preset="card" :title="editing ? '编辑通道' : '新增通道'" style="max-width: 540px">
      <n-form label-placement="top">
        <n-form-item v-if="auth.isPlatformAdmin" label="目标租户" required>
          <n-select v-model:value="form.tenantId" :disabled="!!editing" :options="tenants.map(t => ({ label: t.name + ' (' + t.tenantCode + ')', value: t.id }))" placeholder="选择通道归属租户" />
        </n-form-item>
        <n-form-item label="上游厂商" required>
          <n-select v-model:value="form.providerId" :disabled="!!editing" :options="providers.map(p => ({ label: p.name + (p.scopeType === 'PLATFORM_SHARED' ? '（共享）' : '（私有）'), value: p.id }))" placeholder="选择厂商" />
        </n-form-item>
        <n-form-item label="接入基础地址" required>
          <n-select v-model:value="form.providerBaseUrlId" :disabled="!form.providerId" :options="formBaseUrls.map(b => ({ label: b.protocol + ' · ' + b.normalizedBaseUrl, value: b.id }))" placeholder="选择接入地址" />
          <template #feedback>仅可选择共享或当前租户私有厂商的启用地址。</template>
        </n-form-item>
        ...
        <div style="display: flex; gap: 12px">
          <n-form-item label="连接超时(ms)" style="flex: 1"><n-input-number v-model:value="form.connectTimeoutMs" :min="100" :max="120000" /></n-form-item>
          <n-form-item label="读取超时(ms)" style="flex: 1"><n-input-number v-model:value="form.readTimeoutMs" :min="100" :max="600000" /></n-form-item>
        </div>
