...
public final class UpstreamUrlValidator {
    ...
    public static String endpoint(String baseUrl, String endpoint, String profile) {
        try {
            ...
            if (!(local && isExplicitLocalHost(base.getHost())) && isPrivate(base.getHost())) throw GatewayFailure.runtimeUnavailable();
    ...
    private static boolean isExplicitLocalHost(String host) {
        return switch (host.toLowerCase(Locale.ROOT)) {
            case "localhost", "127.0.0.1", "::1", "[::1]", "host.docker.internal" -> true;
            default -> false;
        };
    }
