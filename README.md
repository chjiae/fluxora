...
- `fluxora-gateway`：Vert.x 5.1.2 数据面，使用 Redis 运行时快照完成 API Key 鉴权、租户模型选路和 OpenAI / Anthropic 原生中继；不依赖 Spring 或 PostgreSQL。
...
- **原生中继与敏感凭证快照**（V13）：Platform 用独立运行时密钥写入 `UPSTREAM_CREDENTIAL` 密文快照；Gateway 不连接 PostgreSQL，按请求临时解密并安全注入 Bearer / `x-api-key` / NONE。OpenAI 与 Anthropic 入口支持非流式和 SSE 同协议中继、模型名回写、下游反压与断连取消。

### 刻意未实现（本轮范围外）
- 跨协议转换、健康检查、自动失败切换
- Token 计费、流式实时扣费、用量统计
- 其他上游端点、连接测试、协议转换/Adapter
- 支付、充值订单、退款、发票
- 凭证自动轮换、失败切换、请求重试
- 短信 / 邮件 / 自助找回密码
- 每个 API Key 独立余额
