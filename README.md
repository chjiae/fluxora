...
- `fluxora-gateway`：Vert.x 5.1.2 数据面，使用 Redis 运行时快照完成 API Key 鉴权与租户模型选路；不依赖 Spring、PostgreSQL 或上游凭证。
...
- Gateway 热路径不会访问 PostgreSQL，且不保存 API Key 明文、ProviderCredential 明文/密文、认证 Header 或 Base URL。Redis 缺失、Manifest/版本不一致、schema 不兼容或 L1 过期时一律失败关闭。
- 当前只支持内部鉴权与 RouteTarget 选择；`/v1/chat/completions` 与 `/v1/messages` 在成功选路后会返回“模型请求转发暂未开放”。本轮不执行真实上游请求、流式、协议转换或计费。

详细设计见 [运行时配置架构](docs/runtime-config-architecture.md)，逐实体刷新规则见[刷新覆盖矩阵](docs/runtime-refresh-coverage-matrix.md)。本地运行时需要让 Platform 与 Gateway 使用相同的 `APIKEY_LOOKUP_SECRET`、`REDIS_HOST`、`REDIS_PORT` 与（如需要）`REDIS_PASSWORD`。
...
- ProviderCredential（上游凭证）：通过 `ProviderChannelCredential` 绑定到一个或多个同租户、同 Provider 的通道；保存 AES-256-GCM 密文、HMAC-SHA-256 去重指纹与脱敏展示。公开 DTO 与 Redis 快照绝不包含明文/密文/指纹/IV/加密版本。
...
V8 / V9 历史迁移已写入 `flyway_schema_history`，但本地开发数据库可以重置后从空库重放 V1 → V12。**生产环境严禁执行**。
...
# 3. 重启 platform：Flyway 自动按 V1..V12 顺序执行迁移
mvn -pl fluxora-platform -am spring-boot:run
...
V10 自身包含 `DROP TABLE IF EXISTS … CASCADE` + `CREATE TABLE` 的完整重建语义，干净库重放路径上 V8 创建旧表 → V9 加宽旧列 → V10 删旧表并建新表；V12 再加入运行时 Outbox、快照版本与凭证池关联，结果一致正确。**不要修改已有 Flyway 文件**（会破坏 Flyway checksum）。
...
模型领域已按 `tenantId + inboundProtocol + tenantModelCode` 构建运行时快照：`TenantModel → TenantModelPrice → ModelRoute → RouteTarget → TenantModelCandidateMapping → ProviderChannelModel → ProviderChannel → ProviderCredential`。
...
2. **DB 中不保存明文**：仅存可见前缀与完整 canonical Key 的 `lookup_hash = HMAC-SHA-256(APIKEY_LOOKUP_SECRET, canonicalApiKey)`；该不可逆摘要用于 Gateway 精确查找。
3. **Lookup Secret 来源**：`fluxora.security.apikey.lookup-secret`，由环境变量 `APIKEY_LOOKUP_SECRET` 覆盖；生产部署必须改为至少 32 字符随机字符串，并让 Platform 与 Gateway 使用同一个值。Secret 绝不进入日志、堆栈、响应、PostgreSQL 或 Redis。
...
6. **网关校验路径**：先做格式预校验，再计算一次完整 Key HMAC；依次读取 API Key、用户、租户的独立 L1 快照，未命中才异步回退 Redis，绝不访问 PostgreSQL。
...
- **运行时快照与 Gateway**（V12）：控制面事务同写 `runtime_outbox`；Projector 以版本化不可变 Snapshot、Manifest 原子切换和 Pub/Sub 精确失效将 API Key、用户、租户、TenantModelRoute 下发到 Redis。Gateway 使用 Caffeine L1、HMAC Lookup 和内存 RouteTarget 选择完成失败关闭的鉴权与选路；不连接 PostgreSQL、不读取/解密凭证，也不执行真实上游转发。
